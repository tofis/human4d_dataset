...
# root = 'E:/VCL/Users/tofis/Data/DATASETS/RGBDIRD_MOCAP_DATASET/Data/Recordings/_ird_recordings/S2/19-07-12-09-26-19/Dump'

resolution = [320, 180]
save_files = True
# show_gt_skeleton = True
show_gt_skeleton = False
# show_pred_skeleton = True
show_pred_skeleton = False
# show_gt_markers = True
show_gt_markers = False
# show_pred_markers = True
show_pred_markers = False
show_rs_markers = True
# show_rs_markers = False

gt_color = (255, 80, 80)
pred_color = (150, 255, 150)
rs_color = (0, 230, 230)

out = os.path.join(root, 'out_rs')
if (not os.path.exists(out)):
    os.makedirs(out)
sample_ids = range(380, 500)
...
# gt3d = numpy.expand_dims(numpy.load(os.path.join(root, 'gposes3d', str(frame_id-rgbd_skip) + '.npy')), axis=0)

pred_joints = numpy.zeros([19 ,3])
gt_joints = numpy.zeros([19 ,3])
pred_markers = numpy.zeros([53 ,3])
gt_markers = numpy.zeros([53 ,3])
# rs_markers = numpy.zeros([100 ,3])

for frame_id in sample_ids:
    rs_markers = numpy.zeros([150, 3])

    ply_id = frame_id - 101 + rgbd_skip
    ply_data_pred = PlyData.read(os.path.join(root, '../plys', 'pred_joints_{0:05d}_0.ply'.format(ply_id)))['vertex'].data
    ply_data_gt = PlyData.read(os.path.join(root, '../plys', 'gt_joints_{0:05d}_0.ply'.format(ply_id)))['vertex'].data
    ply_data_markers_pred = PlyData.read(os.path.join(root, '../plys', 'pred_markers_{0:05d}_0.ply'.format(ply_id)))['vertex'].data
    ply_data_markers_gt = PlyData.read(os.path.join(root, '../plys', 'gt_markers_{0:05d}_0.ply'.format(ply_id)))['vertex'].data
    ply_data_markers_rs = PlyData.read(os.path.join(root, '../plys', 'rs_markers_{0:05d}_0.ply'.format(ply_id)))['vertex'].data

    pred_joints[:, 0] = ply_data_pred['z']
    pred_joints[:, 1] = ply_data_pred['x']
    pred_joints[:, 2] = -ply_data_pred['y']

    gt_joints[:, 0] = ply_data_gt['z']
    gt_joints[:, 1] = ply_data_gt['x']
    gt_joints[:, 2] = -ply_data_gt['y']

    pred_markers[:, 0] = ply_data_markers_pred['z']
    pred_markers[:, 1] = ply_data_markers_pred['x']
    pred_markers[:, 2] = -ply_data_markers_pred['y']

    gt_markers[:, 0] = ply_data_markers_gt['z']
    gt_markers[:, 1] = ply_data_markers_gt['x']
    gt_markers[:, 2] = -ply_data_markers_gt['y']

    rs_markers[:, 0] = ply_data_markers_rs['z']
    rs_markers[:, 1] = ply_data_markers_rs['x']
    rs_markers[:, 2] = -ply_data_markers_rs['y']
    ...
    gt_joints_t = torch.from_numpy(gt_joints).reshape(1, 1, gt_joints.shape[0], gt_joints.shape[1]).permute(0, 3, 1, 2).type(torch.float)
    ...
    pred_markers_t = torch.from_numpy(pred_markers).reshape(1, 1, pred_markers.shape[0], pred_markers.shape[1]).permute(0, 3, 1, 2).type(torch.float)
    gt_markers_t = torch.from_numpy(gt_markers).reshape(1, 1, gt_markers.shape[0], gt_markers.shape[1]).permute(0, 3, 1, 2).type(torch.float)
    rs_markers_t = torch.from_numpy(rs_markers).reshape(1, 1, rs_markers.shape[0], rs_markers.shape[1]).permute(0, 3, 1, 2).type(torch.float)
    ...
    colorz = [color for color in os.listdir(os.path.join(root, 'color')) \
        if str(frame_id) in color.split('_')[0]]
    depthz = [depth for depth in os.listdir(os.path.join(root, 'depth')) \
        if str(frame_id) in depth.split('_')[0]]
    ...
    for view in views:
        gt2d[view] = numpy.load(os.path.join(root, '../', npyz[view]))[frame_id-rgbd_skip]
        gtbbox[view] = numpy.load(os.path.join(root, '../', npybbz[view]))[frame_id-rgbd_skip]
        ...
        gt_markers_view_aligned = transform_points(gt_markers_t, rotation_inv[view], translation_inv[view])
        pred_markers_view_aligned = transform_points(pred_markers_t, rotation_inv[view], translation_inv[view])
        rs_markers_view_aligned = transform_points(rs_markers_t, rotation_inv[view], translation_inv[view])
        ...
        for p in range(pred_markers_t.shape[2]):
            for j in range(pred_markers_t.shape[3]):                
                # keypoints3d[p, j] = gt_joints_view_aligned[0, :, p, j].cpu().numpy()
                # if ("INF" in args.sequence_filename):
                uv = project_single_point_to_uv(pred_markers_view_aligned[0, :, p, j], intr[view])

                if show_pred_markers:
                    color_img = cv2.circle(color_img, 
                        (int(uv[0]), int(uv[1])), 
                        # COLORS[format(j+1, '02d')],
                        radius=5,
        ...
        for p in range(gt_markers_t.shape[2]):
            for j in range(gt_markers_t.shape[3]):                
                # keypoints3d[p, j] = gt_joints_view_aligned[0, :, p, j].cpu().numpy()
                # if ("INF" in args.sequence_filename):
                uv = project_single_point_to_uv(gt_markers_view_aligned[0, :, p, j], intr[view])
        ...
        for p in range(rs_markers_t.shape[2]):
            for j in range(rs_markers_t.shape[3]):                
                # keypoints3d[p, j] = gt_joints_view_aligned[0, :, p, j].cpu().numpy()
                # if ("INF" in args.sequence_filename):
                uv = project_single_point_to_uv(rs_markers_view_aligned[0, :, p, j], intr[view])
        ...
        for p in range(gt_joints_t.shape[2]):
            ...
            if show_gt_skeleton:
                draw_skeleton_joints_19(color_img, keypoints_pred[p], pred_color, thickness=2)    
            if show_gt_skeleton:  
                draw_skeleton_joints_19(color_img, keypoints_gt[p], gt_color, thickness=2)     
            for j in range(gt_joints_t.shape[3]): 
                uv =  keypoints_gt[p, j]
